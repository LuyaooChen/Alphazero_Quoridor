...
class policy_value_network(object):
    def __init__(self, res_block_nums = 7):
        ...
        # Variables
        self.filters_size = 128    # 128 or 256
        self.prob_size = 84
        self.digest = None
        self.training = tf.placeholder(tf.bool, name='training')
        self.inputs_ = tf.placeholder(tf.float32, [None, 7, 7, 6], name='inputs')  # + 2    # TODO C plain x 2
        self.c_l2 = 0.0001
        self.momentum = 0.9
        ...
        with tf.name_scope("policy_head"):
            ...
            self.policy_head = tf.reshape(self.policy_head, [-1, 7 * 7 * 2])
        ...
        with tf.name_scope("value_head"):
            ...
            self.value_head = tf.reshape(self.value_head, [-1, 7 * 7 * 1])
        ...
        self.train_writer = tf.summary.FileWriter(
            os.path.join(os.getcwd(), "quoridorlogs/train"), self.sess.graph)

        # Test summaries
        self.test_writer = tf.summary.FileWriter(
            os.path.join(os.getcwd(), "quoridorlogs/test"), self.sess.graph)
